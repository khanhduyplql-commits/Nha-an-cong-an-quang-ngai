...
export const RestaurantProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [userRole, setUserRoleState] = useState<UserRole>(() => {
    // If URL has table or mode=customer, default to customer
    if (typeof window !== 'undefined') {
      const params = new URLSearchParams(window.location.search);
      if (params.get('mode') === 'customer' || params.get('table')) {
        return 'customer';
      }
    }
    return isManagerAuthenticated ? 'manager' : 'customer';
  });
  ...
  const loginAsManager = (pin: string): boolean => {
    // Accept standard easy PINs for demo/production: 1234, 8888, 6789, 0000
    const validPins = ['1234', '8888', '6789', '0000', '9999'];
    if (validPins.includes(pin.trim())) {
      setIsManagerAuthenticated(true);
      setUserRoleState('manager');
      try {
        sessionStorage.setItem('qr_manager_auth', 'true');
      } catch {}
      return true;
    }
    return false;
